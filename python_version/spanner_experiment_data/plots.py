...
if __name__ == "__main__":
    ...
    # print(ds.point_gen_method.unique())
    # print(ds.spanner_method.unique())
    # index_cols = ['dim', 'point_gen_method', 'input_file', 'spanner_method', 'epsilon', 'n_points', 'n_edges', 'sparseness', 'edges_to_points_ratio']
    index_cols = ['dim', 'point_gen_method', 'input_file', 'spanner_method', 'epsilon', 'n_points']
    # ds.drop(columns=['sparseness', 'edges_to_points_ratio'], inplace=True)
    # index_cols = ['dim', 'point_gen_method', 'input_file', 'spanner_method', 'epsilon', 'n_points']

    ds_non_blind_greedy = ds.loc[(ds['spanner_method'] == 'greedy-non-blind')].set_index(index_cols, drop=False)
    ds_blind_greedy = ds.loc[(ds['spanner_method'] == 'blind-greedy')].set_index(index_cols, drop=False)
    ds_blind_random = ds.loc[(ds['spanner_method'] == 'blind-random')].set_index(index_cols, drop=False)
    ds_quasi_greedy = ds.loc[(ds['spanner_method'] == 'blind-quasi-sorted-greedy')].set_index(index_cols, drop=False)
    ds_quasi_shaker = ds.loc[(ds['spanner_method'] == 'blind-quasi-sorted-shaker')].set_index(index_cols, drop=False)
    ...
    ds_blind_random_bad_ratio_lbf = ds.loc[
        (ds['spanner_method'] == 'blind-random-bad-ratio-lower-bound-first')].set_index(index_cols, drop=False)
    ...
    # print(ds_non_blind_greedy.head(20))
    ...
    ds_all_dims = ds_non_blind_greedy.join(ds_blind_greedy, lsuffix='_nbg', rsuffix='_bg')
    ...
    ycols = ["sparseness_nbg", "sparseness_bg"]
    # # ycols = ["sparseness_random", "sparseness_greedy", "sparseness_blind_greedy"]
    mask_1 = (ds_all_dims['dim'] == 2) & (ds_all_dims['point_gen_method'] == 'unif') & (ds_all_dims['epsilon'] == 0.1)
    aaa = ds_all_dims.loc[mask_1]
    ds_all_dims.loc[mask_1].plot(x="n_points_nbg", y=ycols)
    # # mask_1 = (ds_all_dims['n_points'] == 200) & (ds_all_dims['point_method'] == 'get_exponential_points') & (ds_all_dims['epsilon'] == 0.01)
    # # ds_all_dims.loc[mask_1].plot(x="dim", y=ycols)
    plt.show()
