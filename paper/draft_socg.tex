...
the sparseness column in \cref{tbl:mcgill_original_blind_greedy_spanner_sparseness} shows the fraction of distances that are almost exact.}.
...
and the \bgrdy algorithms in \cref{fig:spanner_sparseness_mcgill_perturbed}
...
and binning). The results are presented in \cref{tbl:emd_lab_caltech}.
...
Alternatively, one could also compute the Earth Mover's distance
on gray value images by considering each image as a distribution on pixels.
The results of this experiment are in \cref{sec:spanners_euclidean_addendum}, since this variant
does not seem to be used in applications.
...
As in \cref{sec:experiment_spanners}, 
...
we see the speed by almost a factor of $10$.
