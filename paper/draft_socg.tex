...
Another instance is a collection of images
equipped with the Wasserstein (or the Earth Mover's) metric \cite{rubner2000earth}.
...
by a factor of 2 to 5, saving hundreds of hours of machine time,
...
On the other hand, spanner constructions based on Well-Separated Pair Decomposition\footnote{ 
for convenience, we repeat the definition of WSPD in \cref{sec:appendix_background}}~\cite{cal-kos-wspd,hm-fast} only compute
...
space, on images with Earth Mover distance, and on persistence diagrams
with $q$-Wasserstein distance $\wsdist{q}$.\footnote{See \cref{sec:appendix_background} for the definition of $\wsdist{q}$.}
...
%$\eps = 0.01$    &   4093  &  0  &&  5331  &  0  &&  16852  &  0\\
...
$\eps = 0.1$    &   $0.19$  &  $0.42$    &&  $618,587$   &  $262,770$    & $0.42$   \\
$\eps = 0.2$    &   $0.11$  &  $0.30$    &&  $618,587$   & $196,309$    & $0.31$   \\
$\eps = 0.5$    &   $0.04$  &  $0.18$    &&  $618,587$   & $112,995$    & $0.18$   \\
\bottomrule
\end{tabular}
    \caption{Results of \bgrdy algorithm on the dataset of 250 images
    with Earth Mover Distance, time in seconds.}
\label{tbl:emd_gtsrb}
\end{table}
...
persitence in dimension 0, since the diagrams in this dimension are larger in size.
...
has been reported to be very expensice~\cite{kerber2017geometry}.

Sparseness values of \bgrdy spanner on the whole \dtype{original} data set
...
For experiments with images, we used the German traffic sign
dataset \cite{gtsrb} and the code for computing the Earth Mover distance
by Doran \cite{doran_pyemd}.
We randomly selected 250 images from the benchmark.
The first two columns of \cref{tbl:emd_gtsrb}
compare the sparseness of \bgrdy and greedy spanners.
...
For $10 \%$ approximation, we can reduce
the running time by more than a factor of $2$,
saving $98$ hours of computation time.
For $20 \%$ approximation,
we speed up the computations by a factor of 3.
...
we look for an approximate neareast neighbor only.
...
Indeed, as the example in \cref{fig:exact_bad_example} (in~\cref{sec:proof_ann}) 
...
\myparagraph{Earth Mover distance}
Let $\mu_P$ and $\mu_Q$
...
distance in plane.
The earth mover distance between $\mu_P$ and $\mu_Q$
is defined as
\[
...
is sent to the point $q_j$, and the earth mover
distance is the smallest total amount of work
needed to transform the measure $\mu_P$
into the measure $\mu_Q$.


Let $I_1$, $I_2$ be two greyvalue images.
We divide each greyvalue of $I_{1,2}$
by the sum of all greyvalues in the corresponding image.
Thus we can interpret $I_i$
...
The earth mover distance between images $I_1$ and $I_2$
is, by definition, the earth mover distance between
