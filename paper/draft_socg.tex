...
\myparagraph{Outline}
We review the properties of spanner algorithms in \cref{sec:back}.
In \cref{sec:blind_spanners}, we introduce different
variants of the blind spanner algorithm. Experimental
results on spanners are presented in \cref{sec:experiment_spanners}.
Then we move to the ANN problem, giving
an outline of the proof of the $O(\log n)$ bound in \cref{sec:ann}.
The full proof can be found in the appendix.
We report our experimental results for ANN in \cref{sec:experiment_ann}.
...
$\eps = 0.4$    &   $1988$  &  $586$  &&  $3088$  &  $1590$  &&  $-$    &  $6383$ \\
$\eps = 0.5$    &   $1744$  &  $483$  &&  $2835$  &  $1233$  &&  $9418$    &  $5146$ \\
$\eps = 0.8$    &   $1691$  &  $368$  &&  $2809$  &  $814$  &&  $-$    &  $3419$ \\
$\eps = 1.0$    &   $1645$  &  $324$  &&  $2780$  &  $672$  &&  $-$    &  $-$ \\
\bottomrule
\end{tabular}
...
\bottomrule
\end{tabular}
    \caption{Results of \bgrdy algorithm on the dataset of 200 images
...
dataset GTSRB \cite{gtsrb} and the code for computing the Earth Mover's distance
by Doran \cite{doran_pyemd}.
We randomly selected 250 images from the benchmark.
The results of the experiment are summarized in \cref{tbl:emd_gtsrb}.
The first two columns compare the sparseness of \bgrdy and greedy spanners.
...
If a $1.5$ approximation is acceptable,
then the \bgrdy algorithm speeds up
the computation by a factor of more than $5$,
and even for a $10 \%$ approximation, it reduces
