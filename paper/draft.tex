...
\begin{algorithmic}
\Function{DiscardPoints}{$L, p_i, r_i, \eps$}
    \ForAll { $p_j \in L$}
       \If { $ \dist(p_i, p_j) < r_j - r_i / ( 1 + \eps) $ }
...
\Function{ApproximateNearestNeighbor}{$P, q, \eps$}
    \State {Initialize list $L$ with a random permutation of $P$}
    \While {$L$ is not empty}
        \State {Set current candidate $p_i$ to be the first element of $L$}
        \State {Compute $r_i = \dist(q, p_i)$}
        \State \Call{DiscardPoints}{$L, p_i, r_i, \eps$}
        \ForAll {$p_j \in L $}
            \State {Compute $r_j = \dist(p_j, q)$}
            \State \Call{DiscardPoints}{$L, p_j, r_j$}
        \EndFor
    \EndWhile
\EndFunction
\end{algorithmic}
