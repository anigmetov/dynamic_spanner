...
%Let $e$ denote the number of edges of a spanner on $n$ vertices;
%in the plots we visualize the ratios $e / { n \choose 2}$ and $e / n$.
%We refer to the first ratio as \textit{density} and to the second one
%as \textit{growth}. Note that 
...
\begin{algorithmic}
\Procedure{Discard\_1}{$L, p_i$}
    \State {$r_i = \dist(q, p_i)$}
    \ForAll { $p_j \in L$}
       \If {  $ \dist(p_i, p_j) > r_i \left(1 + \frac{1}{ 1 + \eps} \right)$ }
           \State {Remove $p_j$ from $L$}
       \EndIf
    \EndFor
\EndProcedure
\end{algorithmic}
...
\begin{algorithmic}
\Procedure{Discard\_2}{$L, w, c$}
    \State {$r_w = \dist(q, w)$}
    \State {$r_c = \dist(q, w)$}
    \Comment {Distances $r_w$ and $r_c$ are cached}
    \ForAll { $p_j \in L$}
       \If { $ \dist(w, p_j) < r_w - r_c / ( 1 + \eps) $ }
           \State {Remove $p_j$ from $L$}
           \Comment {In particular, the worse candidate $w$ will be removed here}
       \EndIf
       \If {  $ \dist(w, p_j) > r_w + r_c / ( 1 + \eps) $ }
           \State {Remove $p_j$ from $L$}
       \EndIf
...
\Function{ApproximateNearestNeighbor}{$P, q, \eps$}
    \State {Initialize array $L$ with a random permutation of $P$}
    \State {$ c \gets \mbox{the first element of }L$}
    \State {Compute $r_i = \dist(q, p_i)$}
    \State \Call{Discard\_1}{$L, c$}
    \While {$|L| > 1$}
        \ForAll {$p_j \in L $}
            \State {Compute $r_j = \dist(p_j, q)$}
            \Comment {Set the worse candidate $w$ to be either $p_i$ or $p_j$}
            \If {$r_j < r_i$ }
                \State {$ c \gets p_j $}
                \State \Call{Discard\_1}{$L, c$}
                \State {$ w \gets p_i $}
            \Else
                \State {$ w \gets p_i $}
            \EndIf
            \State \Call{Discard\_2}{$L, w, c$}
        \EndFor
    \EndWhile
    \State \Return {the unique element left in $L$}
\EndFunction
\end{algorithmic}
