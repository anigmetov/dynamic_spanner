...
outperforms naive computation of pairwise distances or runs slighly slower, depending on the approximation
quality.
...
On the other hand, spanner constructions based on WSPD\footnote{Well-Separated Pair Decomposition; 
for convenience we remind the definition in \cref{sec:appendix_background}} only compute
...
We performed two types of experiments: on points sampled from a low-dimensional Euclidean
space and on persistence diagrams with Wasserstein distance.
Clearly, our model is not meaningful in a Euclidean space, where
...
The persistence diagrams that we used as
realistic data sets,
come from the McGill shape benchmark \cite{zhang2005mcgill}.
...
Sparseness values of \bgrdy spanner on the whole \dtype{original} dataset
...
First of all, we should explain why the timings in brute-force column
depend on $\eps$. Wasserstein distance can either be computed exactly,
by using the Hungarian algorithm, as done in \dionisus, or
it can be computed with the user-given accuracy using the auction algorithm,
as in \hera. Exact computation is very expensive (see \cite{kerber2017geometry}), and in our code for experiments
with spanner algorithms we always compute $\wsdist{q}$ with relative error 0.01.
In order to make the comparison absolutely fair, when we compute a \espanner,
we compare its timing with the timing of \hera called for each pair of diagrams with relative error $\eps$.
Let us emphasize that the diagrams we have are rather small, with a typical cardinality
between 100 and 300 points. Nevertheless, on this data set \bgrdy spanner
provides a substantial improvement in terms of real running time, by a factor of 2 for $\wsdist{1}$.
...
%$\eps = 0.01$    &   4093 &  - &&  5331 &  - &&  16852  &  - \\
$\eps = 0.1$    &   2605  &  1266  &&  3719  &  4085  &&  0  &  13557\\
$\eps = 0.2$    &   2209  &  878  &&  3338  &  2574  &&  0  &  9383\\
$\eps = 0.5$    &   1744  &  483  &&  2835  &  1233  &&  0  &  5146\\
\bottomrule
\end{tabular}
...
in expectation; there is a technicality that prevents using the standard
backwards analysis argument verbatim, but it can be adapted.
...
We also tested higher dimensions and show the results
...
%distance to any of the points inside the heavily shaded ball or outside the lightly shaded ball that are centered 
%at $p_2$, because their lower bounds allow us to discard them. Note that the point $p_5$, which
%is closer to $q$ than $p_1$, also will not be a candidate, and at least one of the points $p_6,p_7,p_8,p_9$
%in the annulus between the dashed and solid circle, which are further from $q$ than $p_5$, will be chosen as $c$. 
%This shows that in our algorithm the distance from the candidate to $q$ can drop \textit{slower} than in the
%brute-force algorithm, thus \cref{thm:ann_bound} does not immediately follow from standard backwards analysis. 
...
\begin{figure}[hb!]
    ...
    \caption{Example of point set where exact nearest neighbour search cannot be accelerated by maintaining bounds.
...
In \cref{fig:ann_illustration} we illustrate this. First, $p_1$ is
...
A persistence diagram is a multiset of points $(x, y)$ above the diagonal
