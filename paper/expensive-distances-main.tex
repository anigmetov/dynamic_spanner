...
\begin{figure}[ht!]
    \includegraphics[width=0.7\textwidth]{\expDistFigPath blind-greedy-dims.png}
...
\end{tikzpicture}
\end{centering}
    \caption{Sparseness of \bgrdy spanner algorithm. The plot is for 0-dimensional diagrams of perturbed McGill data
    $\eps = 0.1, 0.2, 0.5$ with $W_1$ distance.}
...
persistence diagrams, is \bgrdy. Even though \brdnm also substantially reduces
...
\grdy algorithm is used as baseline\footnote{Of course, in the columns of \grdy algorithm
we give its sparseness, i.e. number of edges divided by the number of all distances;
the percent of computed distances is always 100.}.  The first thing to note is that for higher values of $q$
...
\begin{figure}[ht]
    \includegraphics[width=0.7\textwidth]{\expDistFigPath log_dependency_constant_ratio.png}
...
\begin{figure}[ht]
    \includegraphics[width=0.7\textwidth]{\expDistFigPath ann_computed_distances_dimension_dep.png}
