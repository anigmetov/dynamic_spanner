...
outperforms naive computation of all pairwise distances or runs slightly slower, depending on the approximation
quality.
...
\footnote{definition of $\wsdist{q}$ can be found in \cref{sec:appendix_background}} 
...
this is confirmed by the increase in the size of the greedy spanner.
\ignore{
However, the cardinality of the \grdy
...
%The timings are given in Table 2. We compared our approach for a fixed
...
First of all, we should explain why the timings in the brute-force columns
...
as in \hera. Exact computation is very expensive \cite{kerber2017geometry}, and in our code for experiments
with spanner algorithms we always compute $\wsdist{q}$ with relative error 0.01.
To make the comparison absolutely fair, when we compute a \espanner,
...
provides a substantial improvement in terms of real running time, and we expect this advantage
to grow for diagrams of larger cardinality.
