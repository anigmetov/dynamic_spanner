\documentclass[11pt,natbib=false]{acmart}
...
\affiliation{%
  ...
  \streetaddress{Kopernikusgasse 24}
  \city{Graz}
  \postcode{8010}
  \country{Austria}
}
...
namely the construction of a $(1+\epsilon)$-spanner
...
\acmMonth{01}
\acmYear{2020}
\acmConference[SOSA '20]{SIAM Symposium on Simplicity in Algorithms}{January 06--07, 2020}{Salt Lake City, UT, USA}
\acmBooktitle{SIAM Symposium on Simplicity in Algorithms (SOSA '20), January 06--07, 2020, Salt Lake City, UT, USA}
%\acmPrice{15.00}
%\acmDOI{10.1145/3295500.3356188}
...
An alternative algorithm for computing a spanner is based  on well-separated pair decomposition (WSPD).
Let us recall the definition.  Given $t > 1$, two disjoint subsets $A, B$ of a metric space $(\metricspace, \dist)$ are called $t$-\textit{well-separated},
if 
\[
\forall a \in A \,\, \forall b \in B \,\, \dist(a, b) \geq t \max(\diam(A), \diam(B))
\]
A well-separated pair decomposition is a set of unordered pairs of sets $\{ \{A_1, B_1 \}, 
...
On the other hand, spanner constructions based on WSPD~\cite{cal-kos-wspd,hm-fast} only compute
...
space and on persistence diagrams
...
the other two variants of the \brndm.  (see Figure \ref{fig:blind_rbr_variants}). 
}
...
of 350 points. Nevertheless, on this data set \bgrdy spanner
...
and \bgrdy algorithms in Figure \ref{fig:spanner_sparseness_mcgill_perturbed}
...
neighbour. The pseudocode follows.
...
The full proof is given in the next section,
...
%brute-force algorithm, thus Theorem \ref{thm:ann_bound} does not immediately follow from standard backwards analysis. 
...
%worse than the blind greedy variant. There is a consistent, though small,
%difference between the variants that do not force lower bounds first and
%the other two variants of the \brndm. \ignore{ (see Figure \ref{fig:blind_rbr_variants}). }
...
\begin{figure}[ht!]
    \begin{centering}
    \includegraphics[width=0.5\textwidth]{\expDistFigPath blind-greedy-dims.png}
...
\section{Definition of Wasserstein Distance}
