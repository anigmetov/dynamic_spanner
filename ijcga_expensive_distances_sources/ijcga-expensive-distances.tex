...
\address{Institute of Geometry, Graz University of Technology, Kopernikusgasse 24\\
8010 Graz, Austria\,\\
kerber@tugraz.at
}

\author{Arnur Nigmetov}
\address{Lawrence Berkeley National Laboratory, 1 Cyclotron Road\\
...
algorithmic tasks on metric spaces, at least in practice. Our main motivation for this variant
is the Wasserstein distance, for which one can use the auction algorithm by Bertsekas
to guarantee the relative error provided by the user. From our practice, the algorithm terminates much
faster, if the user is satisfied with a $2$-approximation; better approximations, e.g., with relative error $0.1$,
require much more computations.
...
with $q$-Wasserstein distance $\wsdist{q}$.\footnote{See \ref{sec:appendix_background} for the definition of $\wsdist{q}$.}
...
this number is always $\binom{n}{2}$ and for WSPD it is lower bounded
...
behavior.
Finally, an anonymous reviewer kindly pointed us to
the paper \cite{beacons}, which uses the same idea of inferring
lower and upper bounds for unseen distances from the triangle inequality.
The authors of \cite{beacons} proved that it is possible
to approximate a large fraction of pairwise distances.
However, there is no proof that one gets a spanner in the strict sense,
and the problem seems hard.
Therefore, we postpone the theoretical analysis of the spanner construction
...
a pseudometric, since any permutation of pixels does not
